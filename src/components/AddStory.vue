        ...
        <div class="row">
            <form id="form-upload" @submit.prevent="UpoadStory" class="cols-12 md-12">
                ...
                <div class="row form-group">
                   ...
                   <v-select class="col-md-6" v-model="category" :options="['Football','Boxing','Rugby','Hockey','Tennis']" />
                    <br><br><br>
                ...
                <div class="row form-group  cols-9 md-9" >
                    ...
                    <textarea type="text" placeholder="Write a story...." id="story" row="300" v-model="story" rows="10" max-rows="30"></textarea>
                </div>
               

                <div class="row form-group  cols-9 md-9" >
                    <label id="label" for="storylink">Story source link</label>
                    <input type="text" v-model="storylink" placeholder="Story sourcs" id="storylink" >
                </div>

                <div class="row form-group  cols-9 md-9" >
                    <label id="label" for="">Social media link</label>
                    <input type="text" v-model="medialink" placeholder="Social medis links" id="medialink" >
                 </div>
...
export default {
    ...
    data() {
        ...
        UpoadStory(){
            const storageRef= firebase.storage().ref(`Storyimages/${this.imageData.name}`).put(this.imageData);
            ...
            storageRef.snapshot.ref.getDownloadURL().then((url)=>{
                this.imageUrl =url;
                  ...
                  const doc_id = ref.id;
                 db.collection("Stories").doc(doc_id).set({
                        title: this.title,
                        story: this.story,
                        doc_ID: doc_id,
